                                         ...
                                         MS_CommandStatusWrapper_t* const SCSICommandStatus)
{
	...
	if ((ErrorCode = Pipe_Streaming2(portnum,MSInterfaceInfo->Config.DataINPipeNumber, 
				(uint8_t *)SCSICommandStatus, 13, &actual)) > 0)
	{	
		printf("MS_Host_GetReturnedStatus Error[%d]\r\n", ErrorCode);
		return ErrorCode;
	}
                                       ...
                                       void* BufferPtr)
{
	...
	{
		pipe = MSInterfaceInfo->Config.DataINPipeNumber;
	}
	else
	{
		pipe = MSInterfaceInfo->Config.DataOUTPipeNumber;
	}

	if(Pipe_Streaming2(portnum,pipe, (uint8_t*)BufferPtr,BytesRem,&packsize)){
		printf("MS_Host_SendReceiveData error\r\n");
		return 1;
	}
                                   ...
                                   const void* const BufferPtr)
{
	...
	if((ErrorCode = Pipe_Streaming2(portnum, MSInterfaceInfo->Config.DataOUTPipeNumber,
				(uint8_t *)SCSICommandBlock,  31, &actual)) > 0){
		printf("Pipe Streaming2 Error[%d]\r\n", ErrorCode);
		return ErrorCode;

	}
	//printf("Pipe Streaming2 Send %dBytes\r\n", actual);
                                 ...
                                 void* BlockBuffer)
{
	...
	uint8_t ErrorCode;
	//printf("SCSI Read: Lun:%d BlockAddress:%d Blocks:%d BlockSize:%d Blockbuffer:%p\r\n", 
//				LUNIndex, BlockAddress, Blocks, BlockSize, BlockBuffer);
	MS_CommandBlockWrapper_t SCSICommandBlock = (MS_CommandBlockWrapper_t)
		{
