						   ...
						   uint16_t *const pActualTransferred)
{
	...
	if (XferType == ISOCHRONOUS_TRANSFER) {
		...
		//modify by i4season, fix some bug
...
#if 0 //modify by i4season, fix some bugs
	if(pActualTransferred)
		*pActualTransferred = ExpectedLength;
	HcdQHD(HostID, HeadIdx)->pActualTransferCount = pActualTransferred;	/* TODO Actual Length get rid of this */
...
#ifndef __TEST__
	/*add by zhangwei 20170329 for timeout*/
#define HCD_TIMEOUT		1500	
	int32_t curr = (int32_t) Chip_RIT_GetCounter(LPC_RITIMER);
	int32_t final = curr + ((SystemCoreClock / 1000) * HCD_TIMEOUT);
	...
	while ( HcdQHD(HostID, EdIdx)->status == HCD_STATUS_TRANSFER_QUEUED ) {
		...
		if ((final < 0) && (curr > 0)) {
			if(Chip_RIT_GetCounter(LPC_RITIMER) >= (uint32_t) final){
				printf("WaitForTransferComplete HCDTimeout\r\n");
				break;
			}
		}
		else {
			if((int32_t) Chip_RIT_GetCounter(LPC_RITIMER) >= final){
				printf("WaitForTransferComplete HCDTimeout\r\n");
				break;
			}
		}
...
static __INLINE HCD_STATUS EHciHostInit(uint8_t HostID)
{
	...
	memset(&ehci_data[HostID], 0, sizeof(EHCI_HOST_DATA_T) ); /*Modify by zhangwei 20170329*/
	/*---------- USBINT ----------*/
