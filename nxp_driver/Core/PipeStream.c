...
//	printf("pipe=%d\r\n",pipe);
	...
	if (HCD_STATUS_OK == HcdGetPipeStatus(pipehdl)){
		...
		if(num > 1 && num %8 == 1){
			HcdDataTransfer(pipehdl,buffer,(num-1)*512,&PipeInfo[corenum][pipeselected[corenum]].ByteTransfered);
			status = Pipe_WaitUntilOK(corenum);
			if(actual){
				*actual += PipeInfo[corenum][pipeselected[corenum]].ByteTransfered;
			}
			p += PipeInfo[corenum][pipeselected[corenum]].ByteTransfered;
			len -= PipeInfo[corenum][pipeselected[corenum]].ByteTransfered;
			
		}
		HcdDataTransfer(pipehdl,p ,len,&PipeInfo[corenum][pipeselected[corenum]].ByteTransfered);
		status = Pipe_WaitUntilOK(corenum);
		//if(actual){
			//if(buflen != PipeInfo[corenum][pipeselected[corenum]].ByteTransfered){
		//		printf("buflen=%d,PipeInfo[corenum][pipeselected[corenum]].ByteTransfered=%d\r\n",
		//			buflen,PipeInfo[corenum][pipeselected[corenum]].ByteTransfered);
		//}else{
		//	printf("null\r\n");
		//}
		if(status != 0){
